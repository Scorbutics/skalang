...
static std::pair<ska::bytecode::ScriptGenerationService, BytecodeInterpreterDataTestContainer> Interpret(const std::string& input) {
	...
	return std::make_pair<ska::bytecode::ScriptGenerationService, BytecodeInterpreterDataTestContainer>(ska::bytecode::ScriptGenerationService{ *readerI }, std::move(data));
}
...
TEST_CASE("[BytecodeInterpreter] var declaration") {
	...
	auto gen = data.generator->generate(std::move(script));
	auto res = data.interpreter->interpret(gen.script("main").first,gen).variables.back();
...
TEST_CASE("[BytecodeInterpreter] Outside script from file (import) and use") {
	constexpr auto progStr = "var Character184 = import \"" SKALANG_TEST_DIR "/src/resources/character\";var player = Character184.build(\"Player\");var enemy = Character184.default; var t = enemy.age;";
	auto [script, data] = Interpret(progStr);
	auto gen = data.generator->generate(std::move(script));
	auto res = data.interpreter->interpret(gen.script("main").first,gen).variables.back();
