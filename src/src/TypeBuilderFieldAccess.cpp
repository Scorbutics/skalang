...
ska::Type ska::TypeBuilderOperator<ska::Operator::FIELD_ACCESS>::build(const SymbolTable& symbols, const ASTNode& node) {
    const auto symbolObject = symbols[node[0].asString()];
    if (symbolObject == nullptr) {
        throw std::runtime_error("trying to dereference an unknown symbol : " + node[0].asString());
    }
    ...
    const auto fieldAccessed = node[1].asString();
    ...
    if (symbolField == nullptr) {
        throw std::runtime_error("trying to access to an undeclared field : " + fieldAccessed + " of " + node[0].asString() + (symbolObject->empty() ? (" of type " + symbolObject->getType().asString()) :  " of type " + (*symbolObject)[0].asString()));
    }
