...
ska::TypeHierarchy ska::TypeBuilderOperator<ska::Operator::LITERAL>::build(ScriptAST& script, OperateOn node) {
	auto& symbols = script.symbols();
	return TypeBuilderBuildFromTokenType(symbols, node.asNode());
}

ska::TypeHierarchy ska::TypeBuilderOperator<ska::Operator::UNARY>::build(ScriptAST& script, OperateOn node) {
	auto& symbols = script.symbols();
	return TypeBuilderBuildFromTokenType(symbols, node.asNode());
}
