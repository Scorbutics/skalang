...
ska::NodeCell ska::InterpreterOperator<ska::Operator::FUNCTION_CALL>::interpret(OperateOn node) {
	auto* inMemoryFunctionZone = m_memory.zone(node.GetFunctionName());
	assert(inMemoryFunctionZone != nullptr);

	const auto* function = (*inMemoryFunctionZone)(node.GetFunctionName());
	assert(function != nullptr);
	auto astFunction = function->nodeval<ASTNode*>();
	auto operateOnFunction = Operation<Operator::FUNCTION_DECLARATION>(*astFunction);

	//Centers memory on the current function scope
	auto& currentExecutionMemoryZone = m_memory.pointTo(*inMemoryFunctionZone);
